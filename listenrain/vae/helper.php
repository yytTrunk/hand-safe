...
function vae_get_admin($id)
{
    ...
    $admin['work_id'] = \think\Db::name('admin_work')->where(['admin_id'=>$id])->column('work_id');
    return $admin;
}
...
//读取工区
function vae_get_work_group(){
    $admin = vae_get_login_admin();
    $project_id = $admin['project_id'];
    $group = \think\Db::name('work_order')->where(['status' => 1,'project_id' => $project_id])->order('create_time asc')->select();
    return $group;
}
...
//读取文章分类列表
function vae_get_question_cate(){
    $cate = \think\Db::name('question_cate')->order('create_time asc')->select();
    return $cate;
}

//项目
function vae_get_work_type(){
    $cate = [];
    return $cate;
}

//项目安全科科长
function vae_get_safe_role_cate(){
    $cate = \think\Db::name('admin a')
        ->join('admin_group_access b','a.id=b.uid')
        ->where(['state' => 1,'status' => 1,'b.group_id' => 7])
...
//安全员
function vae_get_safe_role_small_cate(){
    $cate = \think\Db::name('user')->where(['role_id' => 4,'status' => 1])->order('create_time asc')->select();
    return $cate;
}

//网格员
function vae_get_safe_grid_cate(){
    $cate = \think\Db::name('user')->where(['role_id' => 6,'status' => 1])->order('create_time asc')->select();
    return $cate;
}
...
//递归排序
function vae_set_cate_recursion($result,$pid=0,$format="L "){
    /*记录排序后的类别数组*/
    static $list=array();
    ...
    foreach ($result as $k => $v){
        if($v['pid']==$pid){
            ...
            /*将该类别的数据放入list中*/
            $list[]=$v;
            vae_set_cate_recursion($result,$v['id'],"  ".$format);
        }
    }
